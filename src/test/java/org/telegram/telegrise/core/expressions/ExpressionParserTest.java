...
public class ExpressionParserTest {
    ...
    void parse() throws IOException, ClassNotFoundException, InvocationTargetException, NoSuchMethodException, InstantiationException, IllegalAccessException {
        ...
        assertEquals("data", parser.parse(expression, resourcePool, String.class, toNode("<tag expression=\"${" + expression + "}\"/>")).generate(resourcePool));

        expression = "handler.getData(update.getCallbackQuery()) + #reference";
        String finalExpression = expression;
        assertThrows(TranscriptionParsingException.class, () -> parser.parse(finalExpression, resourcePool, String.class, toNode("<tag expression=\"${" + finalExpression + "}\"/>")).generate(resourcePool));

        expression = "handler(update.getCallbackQuery())";
        String finalExpression1 = expression;
        assertThrows(TranscriptionParsingException.class, () -> parser.parse(finalExpression1, resourcePool, String.class, toNode("<tag expression=\"${" + finalExpression1 + "}\"/>")).generate(resourcePool));
    }
